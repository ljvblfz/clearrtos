...
static task_handler_t g_handler [TASK_PRIORITY_LEVELS];
...
{
    task_handler_t handler = task_from_priority (1);

    UNITEST_EQUALS (task_suspend (handler), 0);
    (void) task_sleep (100);
    UNITEST_EQUALS (g_task_running, false);
    UNITEST_EQUALS (task_resume (handler), 0);
    UNITEST_EQUALS (task_resume (handler), 
    ...
    UNITEST_EQUALS (task_delete (handler), 0);
    UNITEST_EQUALS (task_suspend (handler), 
        ERROR_T (ERROR_TASK_SUSPEND_INVHANDLER));

    (void) task_create (&g_handler [1], "test", 1, g_stack [1], sizeof (g_stack [0]));
...
void unitest_main (int argc, char *argv[])
{
    ...
    for (prio = 0; prio < TASK_LAST_INDEX; prio ++) {
        ...
        ecode = task_create (&g_handler [prio], "test", prio, g_stack [prio], 
            sizeof (g_stack [prio]));
        if (0 != ecode) {
    ...
    UNITEST_EQUALS (task_create (&g_handler [0], "test", 1024, g_stack [prio], 
            sizeof (g_stack [0])), ERROR_T (ERROR_TASK_CREATE_INVPRIO));
    
    UNITEST_EQUALS (task_create (&g_handler [0], "test", 0, g_stack [0], 
            sizeof (g_stack [0])), ERROR_T (ERROR_TASK_CREATE_PRIOINUSE));

    UNITEST_EQUALS (task_start (g_handler [0], task_suspending, 0), 0);
    UNITEST_EQUALS (task_start (g_handler [1], task_suspended, 0), 0);
    multitasking_start ();
    ...
    for (prio = 0; prio < TASK_LAST_INDEX; prio ++) {
        ...
        ecode = task_delete (g_handler [prio]);
        if (0 != ecode) {
            printf ("Error: prio = %d, errstr = %s\n", prio, errstr (ecode));
            failed |= true;
        }
